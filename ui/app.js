...
(function () {
  ...
  async function boot() {
    ...
    $("protocolSel").onchange = async () => {
      updateParamButtons();
      // 协议一旦切换就写回会话，避免后续刷新（如保存 101 参数）把界面改回旧协议
      if (curSid) await api("update_session", curSid, { protocol: $("protocolSel").value });
    };
    $("btnSave").onclick = async () => {
      // [AGENT_CHANGE_BEGIN] 2026-09-11 工程保存：新建才命名
      const data = await api("get_project");
      // 已有工程文件（打开过的或保存过的）→ 直接覆盖，不再询问名称
      let path = data.project_path || "";
      if (!path) {
        const name = prompt("请输入工程名称（可含路径；默认 .json）", "未命名工程");
        if (!name) return;
        path = /\.json$/i.test(name) ? name : `${name}.json`;
      }
      const r = await api("save_project", data, path);
      if (!r.ok) await appAlert("保存失败", r.error || "");
      else await appAlert("已保存", r.path || "");
      // [AGENT_CHANGE_END] 2026-09-11 工程保存：新建才命名
    };
    async function openSessionParams(forceProto) {
      ...
      if ((forceProto || s.protocol || "104") === "101") {
        ...
        // 海南现场链路地址 1 字节；工程里遗留的 2（旧默认）在海南模式下按未设置处理
        const mode0 = s.link_mode || (s.balanced ? "balanced" : "unbalanced");
        let addrSize0 = Number(s.addr_size || 2);
        if (mode0 === "hainan" && addrSize0 === 2) addrSize0 = 1;
        ...
        const fields = [
          ...
          [`校验`, `<select id="p_serial_parity"><option value="N"${sel("N", s.serial_parity || "N")}>N 无</option><option value="E"${sel("E", s.serial_parity || "N")}>E 偶</option><option value="O"${sel("O", s.serial_parity)}>O 奇</option></select>`],
          ...
          [`链路地址长度(字节)`, `<select id="p_addr_size" onchange="this.dataset.touched='1'"><option value="1"${sel(1, addrSize0)}>1</option><option value="2"${sel(2, addrSize0)}>2</option></select>`],
          ...
          [`链路模式`, (() => {
            const m = mode0;
            return `<select id="p_link_mode" onchange="var a=document.getElementById('p_addr_size');if(a&amp;&amp;a.dataset.touched!=='1')a.value=this.value==='hainan'?'1':'2';">
        ...
        const handler101 = async () => {
          ...
          const data = {
            ...
            // 海南双主站默认 1 字节链路地址
            addr_size: Number($("p_addr_size").value || (linkMode === "hainan" ? 1 : 2)),
